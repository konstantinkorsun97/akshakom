...
export default function Footer() {
  ...
  return (
    <footer style={{ background: '#111', padding: isMobile ? '36px 20px 20px' : '48px 40px 24px' }}>
      <div style={{
        display: 'grid',
        gridTemplateColumns: isMobile ? '1fr 1fr' : '2fr 1fr 1fr 1fr',
        gap: isMobile ? '28px 20px' : '48px',
        marginBottom: '32px',
      }}>

        {/* ЛОГО — на мобильном занимает 2 колонки */}
        <div style={{ gridColumn: isMobile ? '1 / -1' : 'auto' }}>
          ...
          <div style={{ fontSize: '9px', letterSpacing: '3px', textTransform: 'uppercase', color: '#333', fontFamily: '"Jost", sans-serif', fontWeight: 300, marginBottom: '12px' }}>
            {lang === 'ru' ? 'Комиссионный магазин' : 'Комиссиялық дүкен'}
          </div>
          <div style={{ fontSize: '13px', color: '#444', fontWeight: 300, lineHeight: 1.7, maxWidth: '240px' }}>{tr.footer_desc}</div>
        </div>
      ...
      <div style={{ borderTop: '1px solid #1A1A1A', paddingTop: '20px', display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', gap: '8px' }}>
